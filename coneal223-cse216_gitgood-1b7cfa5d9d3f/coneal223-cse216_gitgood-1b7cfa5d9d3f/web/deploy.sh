...
# cp src/index.html $TARGETFOLDER/$WEBFOLDERNAME/index.html
...
# step 5: compile TypeScript files with webpack
npm run build
...
# cp src/*.css $TARGETFOLDER/$WEBFOLDERNAME
# cp src/components/*.css $TARGETFOLDER/$WEBFOLDERNAME

# step 7: set up Jasmine
node_modules/typescript/bin/tsc src/apptest.ts --strict --outFile $TARGETFOLDER/$WEBFOLDERNAME/apptest.js
cp src/spec_runner.html $TARGETFOLDER/$WEBFOLDERNAME
